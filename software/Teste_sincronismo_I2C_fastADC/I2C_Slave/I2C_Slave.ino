...
#define END_PIN 4     // D4
#define SIGN_PIN 2   // A2, porta responsável por ler os sinais analógicos

byte sinal[NPONTOS];   //Variáveis responsáveis por memorizar o valor da tensão gerado por PWM, serão usadas para descobrir o momento de queda de tensão. 
...
void loop() {
  if (partida == 200){                         // Verifica sinal de início de leitura.   
    delay(1); // aguarda Master iniciar envio da onda
    inicia_coleta_ADC();                       // inicia coleta
    while (flag_coleta_terminada_ADC==false){  // aguarda terminar coleta
      delay(1);
    }

    int cont = 0;
    for (int idx = 0; idx < NPONTOS-1; idx++){
      if( (sinal[idx] < 128) && (sinal[idx+1] >= 128)){ // detecta subida (8bits, valor máximo 256)
        kont[cont] = (char)idx;
        cont++;
      }
